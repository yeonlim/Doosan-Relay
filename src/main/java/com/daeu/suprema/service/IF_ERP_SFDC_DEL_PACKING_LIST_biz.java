...
@Service
public class IF_ERP_SFDC_DEL_PACKING_LIST_biz extends WebCalloutUtil {
    Logger logger = LoggerFactory.getLogger(this.getClass());

    @Autowired
    HttpRequestUtil httpRequestUtil;

    @Value("${IF.ERP.SFDC.DEL.PACKINGLIST.PATH}")
    private String IF_ERP_SFDC_DEL_PACKING_LIST;
    ...
    public void execute() {
        ...
        logger.info("=========================== [{}] ===========================", "IF_ERP_SFDC_DEL_PACKING_LIST");
        logger.info("### Requst URL : {}", IF_ERP_SFDC_DEL_PACKING_LIST);

        while (true) {
            prcCnt++;

            // 1. 고객별 할인율 정보 조회 (최대 1000 Rows)
            List<Map<String, Object>> packingListMap = repository.SELECT_PACKING_DEL_LIST(prcCnt);
