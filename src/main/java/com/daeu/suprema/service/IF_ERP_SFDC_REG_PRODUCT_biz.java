...
import com.daeu.suprema.io.IF_ERP_SFDC_REG_PRODUCT.PRODUCT_REG;
...
public class IF_ERP_SFDC_REG_PRODUCT_biz extends WebCalloutUtil {
    ...
    public void execute() {
        ...
        logger.info("=========================== [{}] ===========================", "IF_ERP_SFDC_REG_PRODUCT");
        logger.info("### Requst URL : {}", IF_ERP_SFDC_REG_PRODUCT);

        while (true) {
            prcCnt++;

            // 1. PRODUCT 정보 조회 (최대 1000 Rows)
            List<Map<String, Object>> productListMap = repository.SELECT_PRODUCT_REG_LIST(prcCnt);
            ...
            List<PRODUCT_REG> productList = new ArrayList<>();
            for(Map<String, Object> p : productListMap) {
                productList.add(new PRODUCT_REG(p));
            }
