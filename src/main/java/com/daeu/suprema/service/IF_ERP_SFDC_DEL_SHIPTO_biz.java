...
@Service
public class IF_ERP_SFDC_DEL_SHIPTO_biz extends WebCalloutUtil {
    Logger logger = LoggerFactory.getLogger(this.getClass());

    @Autowired
    HttpRequestUtil httpRequestUtil;

    @Value("${IF.ERP.SFDC.DEL.SHIPTO.PATH}")
    private String IF_ERP_SFDC_DEL_SHIPTO;
    ...
    public void execute() {
        ...
        logger.info("=========================== [{}] ===========================", "IF_ERP_SFDC_DEL_SHIPTO");
        logger.info("### Requst URL : {}", IF_ERP_SFDC_DEL_SHIPTO);

        while (true) {
            // 1. 납품처 정보 조회 (최대 1000 Rows)
            List<Map<String, Object>> shiptoListMap = repository.SELECT_SHIPTO_DEL_LIST(prcCnt);
